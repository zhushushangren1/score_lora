...
本文档面向开发和维护人员，记录 LoRa 裁判记分系统的硬件接线、通信协议、业务状态机、网页逻辑、持久化策略、BOM 和后续优化方向。

面向客户和现场工作人员的操作说明见 [USER_MANUAL.md](USER_MANUAL.md)。项目入口和构建说明见 [README.md](README.md)。

当前状态：第一版已经完成三台裁判端与一台服务端联调，洞洞板焊接、电容/电阻补强、断电重启测试均已完成。服务端和裁判端均保留 `lora-debug [on|off]` 串口诊断命令，默认关闭，用于现场排查 LoRa 收发链路。
...
- `/` 自动跳转到 `/score`。
- `/score` 为显示页，只显示两个队伍名称、总比分和倒计时；页面加载后每 1 秒请求 `/live` 并局部更新比分板，不再整页刷新。
- `/control` 为控制页，提供队伍名称修改、绑定、解绑、下一轮倒计时秒数输入、重置比分按钮；页面加载后每 1 秒请求 `/live` 并局部更新比分板、轮次摘要、裁判状态和未绑定设备列表。刚点击按钮或表单正在提交时短暂停止局部更新，避免打断下一轮/重置提交。
- `/live` 为页面局部刷新接口，返回当前比分板、轮次摘要、裁判表、未绑定设备表 HTML 片段。
- 绑定、解绑、下一轮、重置比分、修改队名使用 HTTP POST，并复用服务端已有串口命令/动作处理逻辑。
- 最近 50 条内存事件日志已接入控制页，使用 RAM 环形缓冲保存，断电或重启后清空。

- `/score` 和 `/control` 动态数据轮询间隔为 1000ms。
- 继续保留显示页 `/`。
- 继续保留控制页 `/control`。
...
- 下一轮倒计时：控制页输入秒数，确认后重置总比分、`roundId++`、清空本轮提交、后台按队列重复广播新轮次状态，并启动显示页倒计时。
