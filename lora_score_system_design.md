...
```text
裁判机 x3
ESP32-S3 + E22-400T22D LoRa UART + TM1637 + 5按键 + 18650

        LoRa 470MHz / UART透明传输 / CSV+CRC16

服务端 x1
ESP32-S3 + E22-400T22D/T30D + WiFi AP + LittleFS网页 + NVS状态保存 + 2实体按钮
...
|---|---|---|---|
| 红 +1 / 红 -1 | `GPIO4` | GND | 短按 +1，长按 -1 |
| 红 +2 / 红 -2 | `GPIO5` | GND | 短按 +2，长按 -2 |
| 蓝 +1 / 蓝 -1 | `GPIO6` | GND | 短按 +1，长按 -1 |
| 蓝 +2 / 蓝 -2 | `GPIO7` | GND | 短按 +2，长按 -2 |
...
| 功能 | ESP32-S3 GPIO | 按键另一端 | 程序动作 |
|---|---|---|---|
| 下一轮 | `GPIO4` | GND | 短按；仅本轮完成后有效 |
| 重置总分 | `GPIO5` | GND | 长按 3 秒后重置总比分 |
...
| 功能 | ESP32-S3 GPIO | 接线 |
|---|---|---|
| LoRa 状态灯 | `GPIO6` | GPIO6 -> 1K 电阻 -> LED 正极，LED 负极 -> GND |
| 系统状态灯 | `GPIO7` | GPIO7 -> 1K 电阻 -> LED 正极，LED 负极 -> GND |
...
```text
BOOT:
  初始化 LoRa UART、TM1637、按键、电池 ADC
  读取 MAC 生成 deviceId
  从 NVS 读取 clientId
  发送 HELLO
...
```text
>= 3.7V：正常
3.5V ~ 3.7V：显示低电量提示但允许使用
< 3.5V：提交前强提示
< 3.3V：建议关机或禁止提交
```

实际阈值要按电池、稳压模块和 ADC 分压校准。
...
8. 接入 TM1637，显示未绑定短码、`J1/J2/J3`、比分、`SEND`、`----`、`Err`、`Lo.B`。
9. 接入 5 个按键，实现短按加分、长按减分、长按提交清零。
10. 接入电池 ADC，实现电量上报和低电量提示。
...
| 可选状态灯 | `5mm LED` + `1K 1/4W 电阻` | LoRa RX/TX、系统状态指示 | 5mm LED | 2 组 | LED 必须串限流电阻 |
...
- TM1637 四位数码管显示字母有限，`SEND`、`Err`、`Lo.B` 需要自定义 7 段码。
